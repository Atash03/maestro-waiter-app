/**
 * Cancel Item Modal Component
 *
 * Modal for cancelling an order item with a reason.
 * Features:
...
export interface CancelItemModalProps {
  visible: boolean;
  itemName: string | Translation;
  quantity: number;
  onConfirm: (reason: string, reasonId?: string) => Promise<void>;
  onCancel: () => void;
  testID?: string;
}
...
function getItemDisplayName(name: string | Translation): string {
  if (typeof name === 'string') return name;
  return name.en || name.ru || name.tm || 'Item';
}
...
export function CancelItemModal({
  ...
  // Get display name
  const displayName = useMemo(() => getItemDisplayName(itemName), [itemName]);
  ...
  const getReasonId = useCallback(() => {
    if (useCustomReason) {
      return undefined;
    }
    return selectedReason?.id;
  }, [useCustomReason, selectedReason]);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      await onConfirm(reason, getReasonId());
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to cancel item');
      setIsSubmitting(false);
    }
  }, [getFinalReason, getReasonId, onConfirm]);
        ...
        >
          ...
          >
            ...
            <View style={styles.header}>
              ...
              <ThemedText style={[styles.title, { color: colors.text }]}>Cancel Item</ThemedText>
              <ThemedText style={[styles.subtitle, { color: colors.textSecondary }]}>
                Cancel {quantity}x "{displayName}"?
              </ThemedText>
            </View>
            ...
            <View style={styles.buttonRow}>
              ...
              >
                {isSubmitting ? <ActivityIndicator size="small" color="#FFFFFF" /> : 'Cancel Item'}
              </Button>
            </View>
          </Animated.View>
        </Animated.View>
